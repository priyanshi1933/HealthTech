    ...
    <div style={{ minHeight: "100vh", background: "#f4f7fb", padding: "2rem" }}>
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        ...
        <div className="row g-3 mb-4">
          {[
            { label: "Total Doctors", value: stats.total, color: "#2563eb", icon: "👨‍⚕️" },
              ...
              {doctors.map((doc) => (
                <div key={doc._id} style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                    ...
                    />
                    <div>
                      <div style={{ fontWeight: 700, color: "#1e293b" }}>{doc.userId?.name}</div>
                      <div style={{ color: "#64748b", fontSize: "0.85rem" }}>{doc.userId?.email}</div>
                      <div style={{ marginTop: "4px" }}>
                        <span style={{
                          ...
                          fontSize: "0.75rem",
                          fontWeight: 600,
                          background: doc.verificationStatus === "approved" ? "#d1fae5"
                            : doc.verificationStatus === "rejected" ? "#fee2e2" : "#fef3c7",
                          color: doc.verificationStatus === "approved" ? "#065f46"
                            : doc.verificationStatus === "rejected" ? "#991b1b" : "#92400e",
                        }}>
                  ...
                  {/* Details */}
                  <div style={{ color: "#475569", fontSize: "0.88rem", lineHeight: "1.8" }}>
                    <div>🏥 <strong>Specialty:</strong> {doc.specialty}</div>
                    <div>🎓 <strong>Qualifications:</strong> {doc.qualifications.join(", ")}</div>
                    <div>⏱ <strong>Experience:</strong> {doc.experience} yrs &nbsp;|&nbsp; 💰 <strong>Fee:</strong> ₹{doc.fee}</div>
                  ...
                  {doc.verificationStatus === "pending" && (
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px", minWidth: "200px" }}>
                      ...
                      <input
                        ...
                        onChange={(e) => setRejectNote({ ...rejectNote, [doc._id]: e.target.value })}
                        style={{
                          padding: "7px 12px",
                          borderRadius: "10px",
                          border: "1px solid #e2e8f0",
