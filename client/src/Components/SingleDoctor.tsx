...
export default function SingleDoctor() {
  ...
  const bookSlot = async () => {
    ...
    try {
      ...
      setEmailPreviewUrl(res.data.emailPreviewUrl ?? null);

      if (res.data.emailPreviewUrl) {
        const stored = JSON.parse(
          localStorage.getItem("emailPreviews") || "{}",
        );
        stored[res.data.data._id] = res.data.emailPreviewUrl;
    ...
    <div style={{ minHeight: "100vh", background: "#f4f7fb", padding: "2rem" }}>
      <div style={{ maxWidth: "850px", margin: "0 auto" }}>
        ...
        >
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              >
                <h2
                  style={{ fontWeight: 800, color: "#1e293b", marginBottom: 0 }}
                >
                  {doctor.userId?.name}
                </h2>
        ...
        {bookingSuccess && (
          ...
          >
            <h5
              style={{ color: "#065f46", fontWeight: 800, marginBottom: "8px" }}
            >
              ✅ Appointment Booked!
            </h5>
            <div
            ...
            >
              <div>
                <strong>Payment ID:</strong> {bookingSuccess.paymentId}
              </div>
              <div>
                <strong>Date:</strong>{" "}
                {new Date(bookingSuccess.slotTime).toLocaleDateString("en-IN", {
                  day: "numeric",
                  month: "short",
                  year: "numeric",
                })}
              ...
              <div>
                <strong>Time:</strong> {bookingSuccess.slotTimeLocal}
              </div>
              <div>
                <strong>Fee Paid:</strong> ₹{bookingSuccess.fee}
              </div>
              <div>
                <strong>Status:</strong> {bookingSuccess.status}
              </div>
            </div>
              ...
              {slotsData.blocked ? (
                ...
                <div>
                  ...
                  >
                    ...
                    >
                      🌍 Showing times in{" "}
                      <strong>{slotsData.patientTimezone}</strong>
                      {slotsData.patientTimezone !==
                        slotsData.doctorTimezone && (
                        <span> (Doctor: {slotsData.doctorTimezone})</span>
                      )}
                    </p>
                    <span style={{ fontSize: "0.8rem", color: "#64748b" }}>
                      {slotsData.slots.filter((s: any) => s.available).length}{" "}
                      slots available
                    </span>
                  </div>
                  ...
                  {selectedSlot && (
                    ...
                    >
                      ...
                      >
                        <div>
                          👨‍⚕️ <strong>Doctor:</strong> {doctor.userId?.name}
                        </div>
                        <div>
                          📅 <strong>Date:</strong> {date}
                        </div>
                        <div>
                          ⏰ <strong>Your Time:</strong> {selectedSlot.time} (
                          {timezone})
                        </div>
                        <div>
                          ⏰ <strong>Doctor's Time:</strong>{" "}
                          {selectedSlot.timeDoctor} ({slotsData.doctorTimezone})
                        </div>
                        <div>
